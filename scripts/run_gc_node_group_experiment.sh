...
BASELINE_DIR="outputs/gc_node_group_ab/baseline/coarse"
CANDIDATE_DIR="outputs/gc_node_group_ab/candidate_interval20/coarse"
...
reset_output_dir() {
  local output_dir="$1"
  case "$output_dir" in
    outputs/gc_node_group_ab/*/coarse)
      rm -rf "$output_dir"
      mkdir -p "$output_dir"
      ;;
    *)
...
run_case() {
  ...
  echo "INFINIGEN_GC_NODE_GROUP_INTERVAL=$interval"
  ...
  env \
    ...
    INFINIGEN_GC_NODE_GROUP_INTERVAL="$interval" \
...
run_case "baseline" "1" "$BASELINE_DIR"
BASELINE_STATUS=$?

run_case "candidate_interval20" "20" "$CANDIDATE_DIR"
CANDIDATE_STATUS=$?
